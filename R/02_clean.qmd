...
To ensure the right amount of application variables are made in the code above it is checked whether there is anything in the column other than NAs.

```{r}
clean_data |>
  summarize(sum(!is.na(`Application_8`)))
```

It isn't until application number 9 that there are only NAs. Therefore 8 application columns are sufficient.
...
To make sure as many administration types as possible are added, the NAs are investigated to add further, missed types to a new vector.\
