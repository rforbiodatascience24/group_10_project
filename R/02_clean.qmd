...
It can be observed, from viewing the data, that multiple of the variables contain more than one value. Therefore, each value does not have its own cell which means the data is not tidy. It is further checked to see if there are any missing values in the data.
...
It is observed that there are no missing values and therefore the data is made tidy, by seperating the values into different variables.
...
clean_data <- clean_data |>
  mutate(
    `Medicine Name` = str_extract(`Medicine Name`, "^[^0-9 ]+"
)
  )
...
### Changing names of variables to same style
...
write_tsv(clean_data, save_path)
```
