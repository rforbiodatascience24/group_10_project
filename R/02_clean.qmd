...
A search is done for the words in the vector and if a word appears in Medicine Name it is added to the new variable Administration_type. To make sure as many administration types as possible are added, the NAs are investigated to add further, missed types to a new vector. In the end there were 333 observations left which were NAs, these different administration types were changed to "Other".

```{r}

t <- c("Injection", "Tablet", "Syrup", "Cream", "Oral Suspension", "Capsule", "Eye Drop", "Liquid", "Suspension", "Inhaler", "Gel", "Drops", "Lozenges", "Eye/Ear Drops", "Expectorant", "Nasal Spray", "Respules", "Eye Ointment", "Soap", "Penfill", "Scalp Solution", "Respirator Solution", "Cartridge", "Eye Drops", "Ophthalmic Solution", "Nail Lacquer", "Infusion", "Drop", "Readymix", "Oral Solution")

updated_routes <- c("Solution", "Lotion", "solution", "Vaccine", "gel", "Inhalation", "Shampoo", "tablet", "Ointment", "Transhaler", "Powder", "Spray")

variable_name <- 'Medicine Name'

data_test <- data %>%
  mutate(
    Administration_route = str_extract(
      `Medicine Name`, 
      paste(c(routes, updated_routes), collapse = "|")
    ) |>
      replace_na("Other")
  )

data_test |>
  select(Administration_route, `Medicine Name`) |>
  filter(str_detect(Administration_route, "Other")) |>
  print()
...
clean_data <- clean_data |> 
  ...
  #Every word consisting of only capital letters are converted to lower case e.g. "Anormal ACG" becomes "Abnormal acg"
...
```{r}
clean_data <- clean_data |>
  mutate(across(starts_with("Side_Effects_"), ~ str_trim(.x, side = "right")))
```
...
clean_data <- clean_data |>
  select(-Composition,-Uses, -`Side_effects`, -starts_with("API_"), -`Image URL`)
```
