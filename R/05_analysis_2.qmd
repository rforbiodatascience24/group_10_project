...
A subset of the cancer data is made to investigate only the 10 most prevalent types of medicine. These medicines were identified through the descriptive statistics and appear to be the only ones exceeding 2% prevalence in the cancer dataset.
...
The subset shows the 10 most prevalent drugs in the cancer data. From the dimension of the data frame, 10 disitinct ID's appear
...
Most_prevalent_medicines_plot <- Most_prevalent_medicines |>
  ...
  ggtitle(str_wrap("Relationship between side effects and ten most commmon cancer drugs", 50)) +
  theme_minimal() +
...
The visualization only yields 8 different types of drug names, which is explained by one of the drugs, Anfoe, appearing three times out of the ten most prevalent, simply in different doses. Hence only 8 unique medicines exceed a prevalence of 2%. The distribution of side effects seems random, if not dominated by a rather high number of side effects. The number of side effects does therefore not seem to be the cause of the popularity of the prevalent drugs.
...
A higher number of side effects also seem to be associated with administration of the drug via injection. To investigate this trend further, the density of side effects related to each of the different administration types is visualized in the cancer data with the aim of finding a general trend. This is done for each of the medicines by using the ID in order to obtain unique observations.
...
ggsave(here("results/Side_Effects_Administration_type.png"), plot = Side_Effects_Administration_type, width = 6, height = 4)

print(Side_Effects_Administration_type)
```

The density plot shows a general trend between Injection and a medium to high number of side effects. The peak at \~20 confirms what is found in the subset of the prevalent drugs. Furthermore, a peak in the density of side effects aorund \~10 is observed for tablets, which is in accordance with tablet-administrated drugs Advancan and Rolimus in the prevalence medicine subset.
...
The data frame shows 10 unique ID's, which can also found in the descriptive analysis. The drug Anfoe appears three times, supporting results from earlier. From the classification of review, the levels of average and excellent are somewhat appearing at the same frequency, 4 and 5 times respectively, whereas the review level poor only appear once. This indicates that a poor review might potentially affect the prescription and/or choice of a drug.
