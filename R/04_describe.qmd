...
It can be observed that the lowest number of side effects is one and the maximum number of side effects is 23. Furthermore, the average number of side effects for the drugs is around 13. The median is observed to be 10. When comparing the median to the mean (accordingly 10 and \~13) it indicates few observations with many side effects skewing the mean to a higher value.

### Mean, maximum, and minimum number of active ingredients

Now, the Count_API variable is investigated to look into the maximum, minimum, and average number of active pharmaceutical ingredients.
...
There are up to six different applications of the different cancer drugs and minimum one application. The average number of applications for the drugs is around 3, which means most drugs can treat more than one type of cancer. When comparing the mean with the median of 2 it indicates a left skewed distribution. This show half of the observations having two or less applications.

\
It is also investigated how many different applications there are and what they are.
