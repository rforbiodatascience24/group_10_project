...
The raw data set investigates the composition, administration type, applications, and side effects of the different medications. Furthermore, the drugs are reviewed by the user and split into poor, average, and excellent reviews by percentages. The sum of three categories of reviews adds up to collective of 100%.
...
The dimensionality of the data is 52.080 x 19, from which we get that the data include 52,080 rows, with 19 variables for each.
...
From dimensionality of the subset, containing only unique rows with 'cancer' related applications, 23 unique cancer applications of appaears in the dataset.
...
**There are 49 different applications of the drugs, which means that 49 different cancer types can be treated with the drugs from this data set.**

### Investigating the different drugs

The number of unique drugs is in the cancer data set to get an idea of how many times the different drugs appear a histogram is made.
...
cancer_data |>
  ...
  ggtitle("Frequency of unique drus in cancer data") +
