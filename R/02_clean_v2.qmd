...
clean_data <- clean_data |> 
  ...
  #Every word consisting of only capital letters are converted to lower case.
  mutate(Uses = str_replace_all(Uses, 
                                "\\b[A-Z]+\\b", 
                                ~str_to_lower(.)))
```
...
Kig på unnest_wider til split split af data application og sideeffects.
...
clean_data1 <- clean_data1 |> 
  mutate(
    Dose_1 = str_extract(API_1, "\\d+"),  # Extract dose for the first compound
    Unit_1 = str_extract(API_1, "(?<=\\d)[a-zA-Z/%.]+"),  # Extract unit for the first compound
    
    Dose_2 = str_extract(API_2, "\\d+"),  # Extract dose for the second compound
    Unit_2 = str_extract(API_2, "(?<=\\d)[a-zA-Z/%.]+")   # Extract unit for the second compound
  )
